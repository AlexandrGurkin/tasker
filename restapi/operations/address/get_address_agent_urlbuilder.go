...
type GetAddressAgentURL struct {
	Agent string
...
func (o *GetAddressAgentURL) Build() (*url.URL, error) {
	...
	var _path = "/address/{agent}"

	agent := o.Agent
	if agent != "" {
		_path = strings.Replace(_path, "{agent}", agent, -1)
	} else {
		return nil, errors.New("agent is required on GetAddressAgentURL")
	}
...
func (o *GetAddressAgentURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetAddressAgentURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetAddressAgentURL")
	}
